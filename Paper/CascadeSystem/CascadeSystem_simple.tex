...
\begin{table}[htbp]
	...
	\label{tab:system-data}
\end{table}
\nomenclature{$P_{ge}$}{Power of generator, W}
...
\nomenclature{$A_{dc}$}{Aperture area of each dish collector, m$^2$}
\nomenclature[G]{$\Delta{}T_{3,2,min}$}{Minimum temperature difference between oil and water in the oil-to-water heat exchanger, K}
\nomenclature{$T_{tc,o}$}{Trough outlet temperature, K}
\nomenclature{$p_{tc}$}{Air pressure in trough, Pa}
\nomenclature{$A_{tc}$}{Aperture area of each trough collector, m$^2$}
\nomenclature{$T_{1.2}$}{Air temperature after heating Stirling engine, K}
...
\nomenclature{$T_s$}{Main steam temperature of turbine, K}
...
\nomenclature{$p_{da}$}{Outlet pressure of deaerator, Pa}
...
A dish collector product of SES\nomenclature[C]{SES}{Stirling Energy System} used in Fraser's dissertation, which is also used in this system, and its parameters are listed in Table~\ref{tab:dishCollector}~\cite{Fraser2008}. 
...
\nomenclature{$dep_{cav}$}{Depth of volumetric receiver cavity, m}
\nomenclature{$d_{ap}$}{Aperture diameter of volumetric receiver, m}
\nomenclature[G]{$\lambda_{ins}$}{Thermal conductivity of receiver insulating layer, W/(m$\cdot$K)}
\nomenclature[G]{$\epsilon_{ins}$}{Emissivity of reciver insulating layer}
\nomenclature[G]{$\delta_{a}$}{Thickness of air tube in volumetric receiver, m}
\nomenclature{$d_{dr,i}$}{Inner diameter of air tube, m}
\nomenclature[G]{$\theta_{dr}$}{Dish aperture angle (0$^{\circ}$ is horizental, 90$^{\circ}$ is vertically down)}
\nomenclature[G]{$\gamma_{dc}$}{Intercept factor of dish receiver}
\nomenclature[G]{$\eta_{shading,dc}$}{Shading factor}
\nomenclature[G]{$\rho_{dc}$}{Reflectivity of dish receiver}
...
\nomenclature{$q_{rad,ref}$}{Reflected radiation by volumetric receiver, W}
\nomenclature{$q_{dr,air}$}{Energy absorbed by air in the dish collector}
...
For a Stirling engine in column $x$\nomenclature[S]{$x$}{Stirling engine in column $x$}, $x$ from $1$ to $n_{1}$, according to Appendix,  
